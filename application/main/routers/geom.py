...
async def geom_score_plus(geom_fid: str, db: Session = Depends(get_db)):
    response = Geom_controller.plus(geom_fid, db)
    return response


@router.get("/score/minus/{geom_fid}")
async def geom_score_minus(geom_fid: str, db: Session = Depends(get_db)):
    response = Geom_controller.minus(geom_fid, db)
    return response


@router.get("/score/{geom_fid}")
async def geom_score(geom_fid: str, db: Session = Depends(get_db)):
